...
export async function loginAdmin(prevState, formData) {
    const actualFormData = formData instanceof FormData
        ? formData
        : (prevState instanceof FormData ? prevState : null);

    if (!actualFormData) {
        return { error: 'እባክዎ እንደገና ይሞክሩ።' };
    }

    const password = actualFormData.get('password');
    if (!password) {
        return { error: 'እባክዎ የይለፍ ቃል ያስገቡ!' };
    }

    const adminSecret = process.env.ADMIN_PASSWORD || 'YourSecurePassword123';

    // Verify against ADMIN_PASSWORD or fallback passwords
    ...
    // Also support database admin check if email is provided and model exists
    const email = actualFormData.get('email');
    if (email && prisma?.admin?.findUnique) {
        try {
            ...
            console.error('Database admin check error:', e);
        }
    }
