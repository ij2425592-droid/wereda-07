...
import { getArticleBySlug, getArticles, getStrapiMediaUrl } from '@/lib/api';
...
export default async function NewsDetailPage({ params }) {
  ...
  const formattedDate = article.publishedAt 
    ? new Date(article.publishedAt).toLocaleDateString('am-ET', { year: 'numeric', month: 'long', day: 'numeric' })
    : '';
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <header className="space-y-4">
          ...
          <div className="flex items-center gap-3 border-y border-slate-200 py-3">
            ...
            <div>
              <p className="text-sm font-bold text-slate-900">{article.author || 'የኮሚዩኒኬሽን ጉዳዮች'}</p>
        ...
        <div className="relative aspect-video w-full overflow-hidden rounded-3xl shadow-md bg-slate-100">
          <Image
            src={getStrapiMediaUrl(article.coverImage)}
              ...
              {relatedArticles.map((rel) => (
                ...
                >
                  <div className="relative w-20 h-20 rounded-xl overflow-hidden shrink-0 bg-slate-100">
                    <Image
                      src={getStrapiMediaUrl(rel.coverImage)}
