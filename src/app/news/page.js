...
export default async function NewsPage() {
  // 1. ከ Strapi ዳታውን በ api.js በኩል በቀጥታ እንቀበላለን
  const strapiArticles = await getArticles();

  const hasArticles = strapiArticles && strapiArticles.length > 0;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="border-b border-slate-200 pb-6">
          ...
          <p className="mt-2 text-sm sm:text-base text-slate-600">
            ከStrapi ዳታቤዝ በቀጥታ የሚጫኑ ወቅታዊ የወረዳው የልማት ዜናዎች እና መግለጫዎች።
          </p>
        </div>

        {/* Strapi ሰርቨር ካልበራ ወይም ዳታ ከሌለ የሚያሳየው ማስጠንቀቂያ */}
        {!hasArticles && (
          <div className="flex items-center gap-3 p-6 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900">
            <AlertTriangle className="w-6 h-6 text-amber-600 shrink-0" />
            <div>
              <h4 className="font-bold text-sm">ዜናዎችን ከ Strapi መጫን አልተቻለም</h4>
              <p className="text-xs text-amber-800 mt-0.5">
                እባክዎ የ Strapi ባክኤንድ በ `http://localhost:1337` ላይ መብራቱን እና የ `Articles` API ፍቃድ (Public Permissions) መሰጠቱን ያረጋግጡ።
              </p>
            </div>
          </div>
        )}
          ...
          <section className="relative overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl">
            <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[400px]">
              <div className="relative h-64 lg:h-auto lg:col-span-7">
                <Image
                  src={getStrapiMediaUrl(featuredArticle.attributes?.coverImage)}
              ...
              <div className="flex flex-col justify-center p-6 sm:p-10 lg:col-span-5 space-y-4">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-400">
                  ...
                  <span>{featuredArticle.attributes?.author || 'ኮሚዩኒኬሽን'}</span>
                  <span>•</span>
                  <Calendar className="w-3.5 h-3.5" />
                  <time>{new Date(featuredArticle.attributes?.publishedAt).toLocaleDateString('am-ET')}</time>
                </div>
                <h2 className="text-xl sm:text-2xl font-bold text-white hover:text-blue-300 transition-colors">
                  <Link href={`/news/${featuredArticle.attributes?.slug}`}>
                ...
                <div className="pt-2">
                  <Link
                    href={`/news/${featuredArticle.attributes?.slug}`}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-xs text-slate-500">
                        ...
                        <time dateTime={article?.publishedAt}>
                          {new Date(article?.publishedAt).toLocaleDateString('am-ET')}
                        </time>
                      </div>
